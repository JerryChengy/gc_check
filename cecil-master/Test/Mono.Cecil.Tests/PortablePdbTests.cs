				...
				Assert.AreEqual (Normalize (@"class B
{
	...
	IL_0001: ret", main);
		...
		public void PortablePdbLineInfo2 ()
		{
			TestModule ("Cs2LuaScript.dll", module => {
				var type = module.GetType ("PetAccedeResultView");
				var main = type.GetMethod ("Init");
				var str = Normalize (Formatter.FormatMethodBody (main));
				Console.Write (str);
				Console.Read ();
			}, symbolReaderProvider: typeof (PortablePdbReaderProvider), symbolWriterProvider: typeof (PortablePdbWriterProvider));
		}
		[Test]
		void FormatCallMethod(ModuleDefinition module)
		{
			TypeDefinitionCollection tdCollection = module.GetCollection ();
			foreach (var item in tdCollection.GetNameCache ()) {
				string strFullName = item.Key.Col2;
				if (item.Key.Col1.Length > 0) {
					strFullName = item.Key.Col1 + "." + item.Key.Col2;
				}
				if (strFullName.Contains("MessageDefine")) {
					continue;
				}
				var lib = module.GetType (strFullName);
				if (lib == null) {
					continue;
				}
				foreach (var method in lib.Methods) {
					...
					Formatter.FormatCallMethodInfo (method);
				}
				foreach (var nestedtype in lib.NestedTypes) {
		...
		public void PortablePdbLineInfo1 (string dllName, string filter)
		{
			MethodCallSet.Instance.Init ();
			TestModule (dllName+".dll", module => {
				StringBuilder sb = new StringBuilder ();
				FormatCallMethod (module);
				TypeDefinitionCollection tdCollection = module.GetCollection ();
				foreach (var item in tdCollection.GetNameCache ()) {
					string strFullName = item.Key.Col2;
					if (item.Key.Col1.Length > 0) {
					...
					if (strFullName.Contains("YishiFreeSellView")) {
						int iii = 0;
						++iii;
					}
					var lib = module.GetType (strFullName);
					...
					Formatter.m_FileName = strFullName;
					
					foreach (var method in lib.Methods) {
						if (method == null) {
							continue;
						}
						string str = Normalize (Formatter.FormatMethodBody1 (method, filter));
				...
				using (FileStream fs = File.Open (dllName + "_" + filter + ".txt", FileMode.Create, FileAccess.Write)) {
					if (null != fs) {
