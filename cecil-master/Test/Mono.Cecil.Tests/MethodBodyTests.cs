		...
		{			
			TestModule ("Cs2LuaScript.dll", module => {
				...
				foreach (var item in tdCollection.GetNameCache()) {
					var lib = module.GetType (item.Key.Col2);
					if (lib == null) {
						continue;
					}
					foreach (var method in lib.Methods) {
						...
						string str = Normalize (Formatter.FormatMethodBody (method));
						//if (str.Contains("box ")) 
						{
							string strLine = item.Key.Col2.ToString () + ":" + method.Name;
							sb.Append ("\n").Append (strLine);
				...
				Console.Read ();
				using (FileStream fs = File.OpenWrite ("out.txt")) {
					if (null != fs) {
