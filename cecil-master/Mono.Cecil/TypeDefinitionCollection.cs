...
namespace Mono.Cecil {
	...
	sealed public class TypeDefinitionCollection : Collection<TypeDefinition> {
		...
		internal Dictionary<Slot, TypeDefinition> GetNameCache()
		{
			return name_cache;
		}
		...
		public int GetCount()
		{
			return name_cache.Count;
		}

		public TypeDefinition GetType(int iIndex, ref string strNP, ref string name)
		{
			int i = 0;
			foreach (var item in name_cache) {
				if (iIndex == i) {
					strNP = item.Key.Col1;
					name = item.Key.Col2;
					return item.Value;
